...
contract Federation {
    ...
    event SLARenewalRequested(uint cspId, uint daysToAdd);
    ...
    mapping(uint => VMTypeConfig[]) public cspVMConfigurations;
    ...
    function allocateResources(uint trustValue, uint qosRequirement, uint maxPrice, address requester) public {
      ...
      function requestSLARenewal(uint cspId, uint daysToAdd) public {
        require(msg.sender == owner, "Only the owner can request SLA renewal");
        require(cspId > 0 && cspId <= nextCSPId, "Invalid CSP ID");

        CSP cspContract = CSP(cspSmartContracts[cspId]);
        cspContract.extendSLA(daysToAdd);

        emit SLARenewalRequested(cspId, daysToAdd);
    }
